...
## The clean frame (no background presupposed)

Change in coupling needs no driver. Each step, a standing P's configuration is re-selected from the realizability field, weighted by **coherence-participation** — the F8 pairing overlap with the surrounding standing. "P drifts toward X" means the realizable-next configurations toward X carry more coherence-participation. This uses no rate, no density, no driver — only a comparison of the participation of alternative next-configurations, which is the realizability-weighting law. (This replaced the earlier "conditioning-strength" language, which smuggled a background measure.)
...
This bounded result brings together the realizability-weighting frame, the candidate reading of mass-as-self-closure as a localized participation concentration, and the reported F11 harmonic field. Scope: static weak-field/linear Newtonian modeling. P22 retains the mass-to-source and inverse-clock bridge, the common `3+1` continuum calculation, source normalization, and `G` as open dependencies.
