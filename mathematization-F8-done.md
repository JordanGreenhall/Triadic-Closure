...
- **[conditional input] The pairing** $h$: M5 constructs a perfect `J`-invariant rational bilinear form, and M6 extends it to a non-degenerate Hermitian form only at the grade of a selected rule calculus closed under the complex scalar operations used. Neither step supplies positive definiteness. This F8 construction therefore assumes both that M6 calculus condition and a separately licensed positive-definite physics pairing (probability-readability); conditional on those inputs, $(V,h)$ is a finite-dimensional inner-product space and may be written $h(x,y)=x^\dagger H y$ for Hermitian positive-definite $H$. M5/M6 is not the warrant for this positivity.
...
- **Scalar and positive-definite pairing inputs** [open / separately owned]: M5 supplies a perfect rational form; M6 supplies a non-degenerate Hermitian extension only relative to a selected closed rule calculus; neither supplies positivity. The calculus actually spent and the positive-definite form used for Hilbert-space structure, orthonormal frames, Cauchy–Schwarz, and Gleason must be exhibited in their owning units. F8 is conditional until both dependencies are discharged.
