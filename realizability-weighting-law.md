...
**Gate 2 — exact `|ψ|²` from the pairing (conditional; prior grade corrected).** Given a separately licensed positive-definite Hermitian physics pairing, Gleason's theorem plus the frame-function argument forces the quadratic form `C(k)=|h(σ,k)|²` on dimension ≥ 3. M5 constructs a perfect rational bilinear form and, conditional on M6, a non-degenerate Hermitian extension; it does not supply positivity. The earlier claim that pairing-origin and conservation already fixed the required positive bilinear therefore spent unearned M5 surplus. Gate 2 remains conditional on both the positive-definite pairing construction and the dimension ≥ 3 restriction.
...
**Verdict.** Gate 1 remains Open, Gate 3 remains undone, and Gate 2 is mathematically strong only after its positive-definite pairing input is separately licensed. The overall grade stays **Registered, not sealed**, now with both the frequency bridge and the positive-pairing dependency explicit.
