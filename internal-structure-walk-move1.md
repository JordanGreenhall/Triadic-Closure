...
- **[established-prior, but flagged provisional]** A D2/D4 lead: the two sealed modes (boson = symmetric, fermion = antisymmetric) give three non-interchangeable pairings **bb, ff, bf**, floated as a candidate source of internal/charge degrees. Never sealed; explicitly "provisional, the live work to firm up."
...
- **(M) mode-pairing — NOT a gauge source.** The mode distinction (boson/fermion) is **J-marked: distinct in kind** — bosons co-occupy, fermions exclude; different exchange statistics, different physical behavior. A transformation mixing modes *changes physical content*, so it is **not** a no-distinction direction. The "doublet {bb, ff}" is not a gauge doublet: bb and ff are physically different (boson-pair vs fermion-pair), not two states of one indistinguishable direction. **(M) is a classification by the already-sealed kind-mark, not an internal gauge charge.** [corrected]
...
- **[corrected]** The **(M) mode-pairing** is **not** an internal gauge factor. It is the already-sealed boson/fermion kind-mark, a registered distinction, not a no-distinction direction. The prior lead is corrected.
