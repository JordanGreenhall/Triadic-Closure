---
title: Frontier Closed-Loop Execution Index
type: process
status: control
updated: 2026-07-16
sources:
  - sm-frontier-closed-loop-execution-structure.md
  - sm-content-smuggle-audit-frontier.md
  - claim-status-vocabulary.md
  - triadic-structure-of-relating-rev-canonical.md
  - entering-a-new-domain.md
  - architectonic-rigor-complete.md
---

# Frontier Closed-Loop Execution Index

This folder is the worker-facing execution layer for the SM frontier.

Critical rule: the numbered folders are **frontier items**, not loops. A loop is the repeated solver cycle applied to one item until PASS, COUNTERMODEL, exact MISSING_PREMISE requiring Jordan/Controller decision, or HUMAN_STOP.

## Governing files

- [Closed-loop execution structure](sm-frontier-closed-loop-execution-structure.md)
- [SM content smuggle audit frontier](sm-content-smuggle-audit-frontier.md)
- [Items 2–4 integrated status](02-04-items-2-3-4-status.md)
- [Item 5 status](05-gauge-architecture-beyond-color.md)
- [Item 6 status](06-spin-helicity-chirality-lift.md)
- [Item 7 status](07-particle-identity-ledger.md)
- [Item 8 status](08-charge-hypercharge-ledger.md)
- [Item 9 status](09-specific-color-dynamics-qcd-quarantine.md)
- [Item 11 status](11-decay-product-registration.md)
- [Item 12 status](12-gravity-full-gr-imports.md)
- [Items 3-10 concept-load/pass ledger](03-10-physics-concept-load-pass-ledger.md)
- [Claim status vocabulary](claim-status-vocabulary.md)
- [Triadic Structure of Relating — canonical raw package](triadic-structure-of-relating-rev-canonical.md)
- [Entering a New Domain](entering-a-new-domain.md)
- [Architectonic Rigor](architectonic-rigor-complete.md)

## Frontier item folders

- [Item 1: Physics registration rule](01-physics-registration-theorem/01-physics-registration-theorem Readme.md) — PASS as control/map; no downstream theory regrade
- [Item 2: Amplitude/readout theorem](02-amplitude-readout-theorem/02-amplitude-readout-theorem Readme.md) — PASS for measured downstream load; frequency=weight remains Open seal gate
- [Item 3: Epsilon / SU(3) bridge](03-epsilon-su3-bridge.md) — native SU(3) Registered, not Sealed
- [Item 4: Same-kind complex carrier and representation type](04-same-kind-carrier-representation-type/04-same-kind-carrier-representation-type Readme.md) — CONDITIONAL-PASS / Registered carrier theorem
- [Item 5: Gauge architecture beyond color: SU(2), U(1), completeness](05-gauge-architecture-beyond-color.md) — split: SU(2)/U(1) Registered source-load PASS; completeness defended posit; EW residuals quarantined
- [Item 6: Spin / helicity / chirality / lift theorem](06-spin-helicity-chirality-lift.md) — PASS for registered spin/helicity/chirality machinery; exact module remains Open
- [Item 7: Particle-identity ledger](07-particle-identity-ledger.md) — PASS as adjudication ledger; native roles separated from parked SM bundles
- [Item 8: Charge / hypercharge ledger](08-charge-hypercharge-ledger.md) — PASS as adjudication ledger; native charge structure Registered, exact values/hypercharge parked
- [Item 9: Specific color dynamics / QCD-operator quarantine](09-specific-color-dynamics-qcd-quarantine.md) — PASS as narrowed quarantine; SU(3) consequences Registered, quantitative QCD quarantined
- [Item 10: Native mass-ruler proof](10-native-mass-ruler-proof/10-native-mass-ruler-proof Readme.md) — NON-PASS proof setup only
- [Item 11: Decay-product boundary](11-decay-product-registration.md) — concise P21 shadow; generic classifier Registered, inverse/products/rates bounded by P21
- [Item 12: Gravity full-GR imports](12-gravity-full-gr-imports.md) — PASS by separation; weak-field Registered, full-GR imports conditional/Open
- [Item 13: Higgs / Yukawa / electroweak / generations / spectrum quarantine](13-higgs-yukawa-electroweak-generations-spectrum-quarantine/13-higgs-yukawa-electroweak-generations-spectrum-quarantine Readme.md) — NON-PASS quarantine maintained
