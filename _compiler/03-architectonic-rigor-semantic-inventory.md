...
Status: corpus-wide adjudicated compiler artifact. Source basis: `architectonic-rigor-complete.md`, the current two-axis epistemology in `claim-status-vocabulary.md`, the canonical Triadic Structure source, the domain-entry materials, the explicit foundation-audit repair concerning smuggled demand, and downstream uses throughout the original wiki. The nine-document reconstruction is excluded.
...
## AR-012 — Construction exhibits genesis and exposes smuggled content

Construction assembles a later concept from already secured materials. An unearned premise, primitive, result, or inherited meaning is a **content smuggle** and must be removed or reconstructed natively.

**Disposition:** retained and made operational.

## AR-012a — Smuggle control also tests imported demands

A **demand smuggle** occurs when a question or explanatory obligation generated by a rival ontology is treated as owed by the framework without showing that the framework's commitments generate it. The repair is to expose the imported presupposition and, where the framework does not share it, dissolve the demand. This diagnosis does not excuse an unanswered live question; the absent or rejected presupposition must be shown.

**Disposition:** integrated from the explicit foundation-audit repair.
...
Map dependency; secure roots by retorsion; construct dependents from secured priors; audit content and demand smuggles; mark selections as selections; use recognition with surplus quarantined for established names; and apply the completeness diagnostic whenever exact totality is claimed.
