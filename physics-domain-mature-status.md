...
| P22 selected weak-field metric model | Registered at bounded static-model scope | local weight algebra Secured in selected F8/F9 construction; additional inverse-clock metric map; conditional analytic `-1/r` Hessian and Poisson form | weight alone does not change support; documentary reports are not Secured warrant; source/profile/metric/clock uniqueness, common `3+1`, source normalization, and `G` Open |
...
19. **P22-F1 — source, participation-profile, and clock-map uniqueness:** the conditional local degree weighting, P17 mass ontology, and selected inverse-clock metric model exist, but no native map uniquely turns an identity-maintaining closure into the needed conserved-flow source, turns profile/weight into metric support, or forces `d tau/dt=d_0/d`. Missing: an occupancy-independent source map; exclusion of bare-degree/no-distance, ambient-clock, and flat fixed-support anisotropic-walk countermodels; and uniqueness over admissible metric/clock maps. Unequal weights over unchanged reachable neighbors change expectation but not support, a null cone, or metric structure. This affects attraction, universality, physical mass sourcing, and every gravity consumer; “mass,” “atomicity,” “participation concentration,” and documentary reports must not hide the selections. Local owner: `p22-weak-field-gravity-as-participation-curvature.md`. Next research action: enumerate source, profile-to-metric, and clock maps and prove uniqueness without selecting by empirical fit.
