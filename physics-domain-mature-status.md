...
| P22 weak-field participation curvature | Registered at bounded static-model scope | conditional local tilt, selected inverse clock, reported geodesic/tidal checks, harmonic exterior, conditional Poisson form | source/clock uniqueness, common `3+1` implementation, scalar source normalization, and `G` Open |
...
19. **P22-F1 — source, participation-profile, and clock-map uniqueness:** the conditional local degree weighting, P17 mass ontology, selected inverse clock, and reported attractive weak-field check exist, but no native map uniquely turns an identity-maintaining closure into the needed conserved-flow source or forces `d tau/dt=d_0/d`. Missing: an occupancy-independent source map, exclusion of bare-degree/no-distance and ambient-clock countermodels, and uniqueness over admissible clock maps. This affects attraction, universality, physical mass sourcing, and every gravity consumer; “mass,” “atomicity,” and “participation concentration” must not hide the selection. Local owner: `p22-weak-field-gravity-as-participation-curvature.md`. Next research action: enumerate source and clock maps and prove uniqueness without selecting by empirical fit.
20. **P22-F2 — common `3+1` continuum curvature implementation:** the retained evidence combines reported static `2+1` geodesic/tidal checks with a separate three-spatial-dimensional harmonic graph calculation. Missing: executable sources, one common `3+1` model, an exhibited closed rule calculus, convergence controls, and the spatial metric response. This affects tensorial curvature, light bending, and P24; complementary calculations must not be summarized as a completed `3+1` metric theorem. Local owner: `p22-weak-field-gravity-as-participation-curvature.md`. Next research action: under a separate research authorization, run one source-defined `3+1` convergence study with independent walk and metric specifications.
21. **P22-F3 — scalar source normalization and `G`:** the harmonic Green-function construction conditionally supplies the Poisson form and geometric `4 pi` factor, but the graph-source-to-physical-source map, calibration, sign normalization, and `G` are Open. This affects quantitative force magnitude, Einstein coupling, horizons, and cosmology; defining `M=A/(Gd_0)` must not be mistaken for deriving `M` or `G`. Local owner: `p22-weak-field-gravity-as-participation-curvature.md`. Next research action: after the source and P23 constructions, derive dimensionless source normalization before empirical calibration.
