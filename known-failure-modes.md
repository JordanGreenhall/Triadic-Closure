...
Always consult [[supersession-map]].
...
Do not use `locked actual` as a live status. It is deprecated/disfavored because it hides the proper standing. Translate raw-package `locked actual` language through [[locked-actual-decrement-map]] into Open, Conjectured, Registered, or Registered and Sealed.
...
- selection → forced;
- raw-package `locked actual` → live status without decrementing through [[locked-actual-decrement-map]].
...
See also: [[agent-execution-rules]], [[supersession-map]], [[corpus-lineage]].
