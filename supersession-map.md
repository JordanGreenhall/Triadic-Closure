...
- Earlier: prose F8 material inside `mathematization-F8-F11.md`.
- Current: `mathematization-F8-done.md`.

Execution rule: use the explicit construction/derivation/uniqueness version in `F8-done`.

### D3 color forcing

- Earlier: `d3-color-forcing-argument.md`.
- Later correction: `d3-color-forcing-argument-v2.md` says the first version was rebutted on its central step.
- Later status: `gauge-structure-result.md` says SU(3) is unconditional for same-kind color mediation because the earlier selection (A) is discharged.

Execution rule: if discussing SU(3), begin from [gauge-structure-result](gauge-structure-result.md), not the older conditional theorem.

### Conditional SU(3 theorem

- Earlier: `d3-su3-conditional-theorem.md`.
- Current: `gauge-structure-result.md`.

Execution rule: the conditional theorem is historical/provenance unless specifically analyzing how the unconditional result emerged.

### Mediation-carrier lineage

- Earlier: `d3-as-established.md`, `d4-as-established.md`, and parts of `internal-structure-walk-move1/2.md`.
- Current/rebuilt: `physics-walk-d1-d5-consolidated.md`, plus `gauge-structure-result.md`.

Current formulation: color is the **This-flattening / interior triadic distinction**, not a guessed mediation carrier.
...
Execution rule: use P22 for every weak-field gravity claim and grade. F8/F9 establish the displayed local participation-weight algebra but do not by themselves derive P22's further geometric identification; preserve that identification as P22's selected/Conjectured framework bridge rather than settling it through an imported support ontology. Do not infer a physical source from P17 mass language, infer the exact inverse clock from atomicity alone, identify the distinct `2+1`, underspecified `0.9993`, and F11 `0.997/0.938` reports, treat them as rerun proof, or claim source normalization, `G`, a complete common `3+1` metric, or full GR.

### P23 stress-energy source standing

- Earlier integrated source: `mathematization-F11.md` equated a general multi-stream second moment with one rank-one `rho V V` tensor, treated edge amplitudes as source weights, and claimed tensor conservation from scalar continuity plus absent-script reports.
- Earlier concise status: `stress-energy-three-offices.md` Registered three “independent covariant signature classes” while leaving the With-mode merely unpopulated.
- Current: [P23 Stress-Energy as a Three-Office Source](p23-stress-energy-as-three-office-source.md) Secures the conditional second-moment symmetry, observer-relative `rho/q/S` decomposition, and one exact selected two-stream population example. Sector vocabulary is Registered for an independently licensed physical source. The exact office alignment is Conjectured/Unregistered because P4 gives From/With index blocks while P17 gives a This–From energy relation. Native vectors/weights, general stress, coefficients, and conservation remain Open/Conjectured.

Execution rule: use P23 for every stress-energy construction and grade. Do not collapse a multi-stream sum to one rank-one tensor, call a complex amplitude a real source weight, infer tensor conservation from scalar continuity, call three observer-relative sectors three components or invariants, treat the selected With-stress example as arbitrary matter, or infer P22 normalization/P24 field equations.
...
Package-era opening reconstruction. Extremely superseded by the later conversation and result modules; use only as provenance for what was later revised, not as active doctrine. Any claim sourced only to Φ must be rechecked against later artifacts before use.
