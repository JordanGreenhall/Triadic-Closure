...
> **F3 terminology control:** this document's horizontal address-length is **succession count**, not depth. Depth is reserved for vertical coinductive office-decomposition; altitude names horizontal layer position.
...
- **(M4) Finite-chain additive grading.** [M3 prior] In finite endpoint-bounded oriented chains, identical positions are distinct by relational role. Finite positive order-characters add by tag-free concatenation; their formal-difference completion supplies an additive integer grading with inverse by swapping difference roles. The construction is Secured at that reach. `Positive natural numbers under addition` and `integers` are Registered only for these additive cores; multiplication, an empty positive order-character, unordered cardinality, and further legacy surplus are not inherited.
- **(M5) ρ-coherence; monotone conservation.** [forced-prior, 0.5, 0.6] Recursive-coherence ρ holds finite-stage pieces into the whole and composes passages up to coherence. Registered distinctions are conserved monotonically — once registered, permanent; identifications provisional.
...
- N carries the inherited content at each node: the office-survey (0.1), J (0.2), the two sectors (0.3), and the pairing at its certified grade (0.4). Canonical normalization unit M5 supplies a perfect `J`-invariant rational bilinear form; canonical unit M6 extends it to a Hermitian form only relative to a selected rule calculus closed under the scalar operations used. Positivity/Hilbert structure is not inherited. These ride on the standings only at those grades and conditions; they are not extra structure added to the graph.
...
This gives the standing discipline for all downstream gravity work: **every quantity must pass (P2); rate and density are banned by (C2.1); coherence-participation, address-distance, succession count, and sector-content are the admissible primitives.**
...
- **Succession count (the From-count).** The length of the address — how many turn-overs deep a standing is. Oriented (one-way, M2), hence ordered. [This will be time; F2 establishes its atomicity.]
...
**(P3) [proved] Succession count and address-separation are distinct intrinsic measures, both definable on N, neither presupposing the other or any background.** Succession count is a property of a single address (its length); address-separation is a relation between two addresses (connecting-chain-length). Both are counts of edges/turn-overs (M1, M4) — admissible by (P2). They are distinct because succession count is monadic-ordered (one address, oriented) and separation is dyadic (two addresses); §F2 shows succession count is atomic, and the From/With distinction (Treatise §7.4a) makes succession count the oriented From-count and separation the symmetric With-count. ∎

---

## F2 — Step atomicity

**Claim to prove:** the step (one conditioning = one succession-count increment) is **atomic** — there is no fractional conditioning, no continuum of sub-steps between a standing and the standing it conditions. The temporal substructure is genuinely discrete, and this is *forced*, not chosen.

### F2.1 The proof

**(P4) [proved] One conditioning is atomic.** A conditioning is one turn-over (M1): the taking of one aspect of one standing as the unit for the next. A turn-over is a single office-act — by the Treatise's stride discipline (no fourth operation; a determination is one act over standing materials), it does not decompose into sub-takings that are themselves turn-overs of the same build, because a "half-turn-over" would be a taking that has neither completed (no new standing yet) nor not-begun (no standing to be unit of) — it would be a standing that both is and is not at unit-altitude, which the closure discipline forbids (a standing either stands, at a definite address and succession count, or it is not a standing). Hence a conditioning is indivisible: it either has occurred (a new address at count n+1 stands) or it has not (only the count-n standing stands). There is no intermediate. ∎

**(P5) [proved within the exhibited address construction] Succession count is discrete and orientation-sourced.** By (M4), succession count records one-way takings along an address in the constructed ℤ-grading. P4 supplies atomic steps: a turn-over has no intermediate standing within this construction. Succession count therefore takes values in the oriented discrete grade (ℤ⁺ along an address), with no constructed value between successive counts. This conclusion does not depend on the withdrawn claim that symmetric multiplicity has no arity. ∎
...
2. *From the exhibited grading (M4, P5):* the available address-count is oriented succession and is intrinsically discrete (ordinal); M6 confirms that no background continuum is inherited to supply intermediate values. This establishes discreteness of the constructed succession grading. It does not establish that orientation is the only possible source of every form of cardinality; unordered cardinality remains Open. ∎
...
- **Not established here (correctly deferred):**
  - Whether the *large-scale* temporal structure admits a rule-presented continuum description belongs to the continuum-limit gates (F10/F11), not here. Canonical M6 supplies cut and scalar schemas at a selected closed rule-calculus grade; it does not prove that coarse-graining this discrete succession count yields smooth time or a manifold. F2 establishes only the discrete substructure, so no discrete/continuous conflict is decided here.
...
**Proved here:**
- **F1:** N is the background-free horizontal build of the terminal object (P1); the admissibility criterion for quantities (P2), with rate/density banned (C2.1) and coherence-participation admitted (C2.2); succession count and address-separation as the two distinct intrinsic measures (P3).
- **F2:** the step is atomic (P4); succession count is discrete and orientation-sourced (P5); discreteness is forced, including by the gate's exclusion of the completed continuum at step level (P6).
