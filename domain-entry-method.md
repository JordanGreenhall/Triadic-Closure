...
This is the governing F4 method for moving from a standing ground into a successor domain without borrowing the successor's unconscious metaphysics. The detailed construction lives in [The Triadic Structure of Relating — Canonical Foundation](triadic-structure-of-relating-rev-canonical.md), Parts 3 and 8. This page states the operating boundary and C2 record.
...
| The successor inherits the standing ground whole as constraint. | **Secured by dependency**. | Every certified prior distinction remains binding at its inherited grade. Inheritance does not require every feature to be actively instantiated or spent in every successor content. | **Native** |
...
Before a familiar domain concept does load-bearing work:

1. name the inherited concept and the content or demand it would smuggle;
2. refuse that surplus as a premise;
3. reconstruct the needed core from already earned framework structures;
4. test the reconstruction against a concrete case;
5. assign its framework C2 record;
6. register the familiar name only over the Secured mapped core, with all surplus quarantined.
...
“Inherited whole” is a constraint rule:

- no certified prior distinction may be contradicted, silently dropped, or downgraded;
- each inherited feature retains its own framework state, conditions, and debts;
- a successor may spend only features actually licensed by the gate-package;
- every expenditure must show where the feature bites;
- a feature may be inherited yet inactive at a particular phase.

Thus the ground is not a buffet of optional premises, but neither is inheritance a demand that every feature actively appear in every successor actual. A walk audits each inherited feature as **bites / does not bite / unresolved**, and explains the result.
...
The D-series is a selected path-schema at recognized sufficiency, not an inventory of every possible phase.
...
| D5 — Variation-space | Realizability(S) stands at unit-altitude as a coherent field of admissible variation carrying the D4 invariant. | Requires D4 content; metric and magnitude remain quarantined. |
...
The framework phase labels are Native. Several familiar technical names are licensed only over bounded cores:

| Inherited name | Licensed framework core | Quarantined surplus | Registration |
| --- | --- | --- | --- |
| transformation | admissible passage between configurations under the boundary | group action, dynamics, continuity, symmetry | **Registered** |
| invariant | basis-registered sameness across lineage-related passages | conserved magnitude, Noether structure, symmetry group | **Registered** |
| field | Realizability(S) taken at unit-altitude as a coherent variation-space | metric, magnitude, manifold, physical field equations | **Registered** |
| persistence | invariant-preservation across allowed variation | endurance through time, substance, permanence | **Registered** |

Registration contributes no warrant. The borrowed name may do no work beyond its licensed core.

## 7. Domain handoff

D7 is a passability location, not the terminus or goal of a domain. A candidate is selected by the characteristics required to provision the proposed successor boundary, together with its ambient partners. If no location presently satisfies those characteristics, the finding is “not yet mature.”

The stronger claim that every successor inhabits the residue left by its predecessor and that D7 maximizes the successor's recursion is not part of the general F4 method. It remains a conjectural generalization from the physics→chemistry case until independently constructed across further gates.

## 8. Domain-specific work remains downstream

Physics may propose self-transforming / invariant / coherent structure as its boundary and may read that constraint as threefold causation. Chemistry may propose a closure-completion boundary over persistent physical structures. Those are domain-specific gate records, not generic F4 results.

Likewise, time, space, color, particle, field in the physical sense, boson, fermion, atom, bond, and carbon retain their owning-domain warrant and registration records. F4 neither derives nor upgrades them.
