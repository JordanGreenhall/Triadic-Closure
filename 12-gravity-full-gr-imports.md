...
|---|---|---|
| Local anisotropic participation weighting | **Secured within selected F8/F9 construction** | Changes relative weight over available alternatives; does not by itself change support, a null cone, or the metric. |
| Selected weak-field static `g00` metric model | **Registered at bounded model scope** | Requires an additional profile-to-clock/metric map; inverse clock and source map are not uniquely forced. Absent-script finite values are documentary reports only. |
...
Dependent pages may use only P22's bounded selected static metric model, conditional analytic `-1/r` consequences, harmonic exterior, and conditional scalar Poisson form, with P22's explicit conditions. They may not turn weighting alone into changed support or curvature, promote documentary reports into Secured warrant, identify the two three-dimensional reports, claim script reproduction, claim a complete `3+1` metric, derive a physical source, or derive exact `G`. Stress-energy belongs to P23. The full Einstein equation must be marked Registered-conditional on the G1-conditioned second-order route plus Lovelock-style tensor rigidity, not Sealed. Nonlinear/strong-field results and tensor completeness remain Open. Lambda claims remain P28-owned.
