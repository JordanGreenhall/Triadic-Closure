...
## The cardinal discipline (applies to all three moves)
...
## Move 1 — Dependency position: what should we be looking for here?

**Where in the dependency hierarchy are we, and therefore which physics structures are candidates for emergence at this stage?**
...
## Move 2 — Combination from the standing: what do our actuals compose into?

**Take what is actually standing and ask what it combines into**, using the framework's own operations. This is the dead-center stage operation (for the relational stages): bring standing structures into relationship and find the determinate content.

1. For pairs (and small combinations) of standing structures, ask the two D-stage questions:
   - Is there **determinate With-content** that brings them into structured coherent relationship?
   - Are there **Thises** that compose them into a new unity (closure)?
   And ask which **operation** does it: From, With, This, ρ, J, the pairing, the linear structure — which of these converts the standing pieces into a new physics-relevant structure.
2. **The search is bounded by Move 1's portfolio.** The combinations explode; we do not search exhaustively. We let the dependency portfolio (the physics structures that matter) be the **orienting basis** — we look at the combinations that produce portfolio-items, not all combinations. Move 1 (what matters) orients Move 2 (what combines).
3. Grade what emerges; track what content it tacks onto an already-registered essential form (Move 1's maturation tracking).

**Move 1 and Move 2 are complementary directions.** Move 1 is top-down (from the physics portfolio: what should appear). Move 2 is bottom-up (from the standing: what does combine). Run both; they cross-check. Move 1 catches "we should have X by now — do we?"; Move 2 catches "these two standing things obviously relate — into what?" The gravity error was a Move-2 failure (two standing structures, Hamiltonian and spatial degrees, obviously in relationship, and the relationship waved off).
...
## Why all three, every stage

- Move 1 alone risks reverse-fitting (looking for known physics and placing it).
- Move 2 alone risks aimless combination (the explosion) and missing structures whose relationship isn't locally obvious.
