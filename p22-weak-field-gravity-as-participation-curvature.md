...
### Governing sources by exact scope

1. [Realizability Weighting Law](realizability-weighting-law.md) and [Amplitude / Readout Theorem](amplitude-readout-theorem.md) govern what a normalized coherence-participation weight does and does not mean. Weight is not observed frequency or a calibrated rate.
...
- `mathematization-F10-status.md` is historical/adversarial. Its background-time drift and two-clock sign ambiguity remain useful failure tests, but its conclusion that F10 is simply unfinished is superseded by the later bounded construction.
- `mathematization-F8-F11.md` is an earlier synthesis. Dedicated F8–F11 sources and this normalized page govern current use.
- Claims that the simulations prove an unrestricted curvature theorem, that Poisson's coefficient is fixed “with no free choice,” or that a mass is automatically the required graph source exceed the bounded warrant and are superseded here.
- The absent scripts named by the source files are not treated as executable evidence. Their reported outputs remain retained-source evidence only.

## 1. Conditional discrete input

Let `e_i` be an elementary next-coupling and let `d_i` be the target node's conditioning degree. Within the positive-pairing, normalized-frame, and conserved-flow standing construction inherited from M3/F8–F9,
...
The displayed F10 construction selects atomic proper advance to scale inversely with the local profile:
...
For `d = d_0 + delta d` with `|delta d| << d_0`, the source records

```text
Phi = -(d - d_0) / d_0
```

at first weak-field order, so `g_00` has the standard static weak-field form up to convention. The negative sign yields attraction toward increasing `d` in the selected model.
...
- near-static walk/geodesic acceleration ratio approximately `1.01` after a convention-factor correction;
- radial separation increasing from `6.0` to `16.1`;
- transverse separation decreasing from `6.0` to `4.8`;
...
- the named scripts are absent and were not rerun during normalization;
- the reported F10 simulation scope is static `2+1`, whereas exact physical dimension is `3+1` and the F11 harmonic calculation is spatially three-dimensional;
- discretization residuals remain;
...
### P22-F1 — Source, participation profile, and clock-map uniqueness

**Open question:** What native map takes a P17 identity-maintaining closure to a localized conserved-flow perturbation and forces both the attractive sign and the exact inverse clock `d tau/dt=d_0/d`?

**Current strongest result:** the conserved-flow construction gives the conditional local degree weighting; P17 gives mass-as-maintenance; the selected inverse-clock model gives attractive static `g00` and agrees with the reported weak-field walk check.
...
### P22-F2 — One common `3+1` continuum curvature implementation
...
**Missing:** the graph-source-to-physical-source map, units/ruler calibration, sign convention fixed jointly with P22-F1, and P23-compatible source accounting.
